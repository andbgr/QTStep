...
displayname="QTStep Classic Translucent $colorscheme"
...
for i in widgets/*.classic.svg; do
	mv -v $i $(echo $i | sed 's/\.classic\.svg/.svg/')
done
rm -v widgets/*.gradient.svg
...
for file in widgets/*.svg; do
	sed -i '/#808080/s/fill-opacity:1/fill-opacity:0.5/' $file
	sed -i '/#909090/s/fill-opacity:1/fill-opacity:0.5/' $file
	sed -i '/#818181/s/fill-opacity:1/fill-opacity:0.333/' $file
	sed -i '/#919191/s/fill-opacity:1/fill-opacity:0.333/' $file
